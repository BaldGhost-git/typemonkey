...
class Word with _$Word {
  ...
  Word deleted() {
    ...
    if (realWordLength < currentCharIndex + 1) {
      final newWord = word.substring(0, currentCharIndex);
      return copyWith(
        charState: newState.take(currentCharIndex).toList(),
  ...
  CharState? get prevCharState =>
      currentCharIndex < 0 ? null : charState[currentCharIndex - 1];
  ...
  int get countCorrectChar =>
      charState.where((state) => state == CharState.correct).length;

  int get countTypedChar =>
      charState.where((state) => state != CharState.untyped).length;
}
